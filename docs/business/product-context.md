...
- `tui attach` now prints the transcript into terminal scrollback above a borderless managed prompt surface instead of clipping conversation to an internal room body, so messages are not hidden behind an in-app viewport and the send target stays explicit in room chrome rather than a separate pane
...
- `tui attach` can now optionally surface normalized transient live provider progress inline in the chat body when the operator opts in with `--reasoning`, including reasoning-style summaries when a provider exposes them; the room now keeps each completed turn's reasoning block in room-local transcript state so planner, reviewer, writer, delegated sandbox implementation turns, and later turns remain inspectable together, while canonical session history still excludes that transient provider output
