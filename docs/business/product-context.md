...
- `init` now lets an operator bootstrap a workspace-local `crew_agents/` catalog in any directory by copying the installed fallback catalog that `install.sh` refreshes from the repository `crew_agents/` directory, so reinstalling after agent edits changes what new workspaces receive
...
- `tui attach` now keeps the transcript inside the borderless managed room body with width-aware wrapping, so operators can read the live conversation in one place while the compose area stays pinned at the bottom
...
- `tui attach` transcript rendering now stays intentionally plain: grouped sender blocks, sender colors, and inline colored `@agent` mentions remain, while alternating dark card backgrounds and extra transcript padding are avoided so resize behavior stays smooth
- `tui attach` can now optionally surface normalized transient live provider progress in the managed room body when the operator opts in with `--reasoning`, including reasoning-style summaries when a provider exposes them; that progress is strictly live-only and disappears once the turn completes or errors, while canonical session history and printed transcript output still exclude that transient provider output
